...
export function printReceipt(order, restaurantName = 'PHO BAMBOO') {
  ...
  const rows = items
    .map(
      (i) =>
        `<tr><td style="padding:2px 0;">${i.quantity}× ${i.name}</td><td style="text-align:right;padding:2px 0;">${(i.price * i.quantity).toFixed(2)}€</td></tr>`
    )
    .join('');

  const html = `<div style="font-family:'Courier New',monospace;width:280px;font-size:13px;color:#000;">
    <div style="text-align:center;">
      <div style="font-size:18px;font-weight:bold;">${restaurantName.toUpperCase()}</div>
      <div style="font-size:11px;">Puteaux - La Défense</div>
    </div>
    <hr style="border:none;border-top:1px dashed #000;margin:8px 0;">
    <div>Commande <strong>#${order.dailyNumber || '-'}</strong></div>
    <div>${order.table ? 'Table: <strong>' + order.table + '</strong>' : 'À emporter'}</div>
    <div style="font-size:11px;">${new Date(order.createdAt).toLocaleString('fr-FR')}</div>
    ${order.phone ? `<div style="font-size:11px;">Tél: ${order.phone}</div>` : ''}
    <hr style="border:none;border-top:1px dashed #000;margin:8px 0;">
    <table style="width:100%;border-collapse:collapse;">${rows}</table>
    ${order.notes ? `<div style="margin-top:6px;font-size:11px;">Note: ${order.notes}</div>` : ''}
